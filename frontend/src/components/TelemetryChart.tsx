...
type Props = { series: Series[]; loading: boolean; error: boolean; dateRange: { start: string; end: string }; title?: string; onRetry: () => void }
...
export function TelemetryChart({ series, loading, error, dateRange, title = 'Telemetry trend', onRetry }: Props) {
  ...
  useEffect(() => {
    ...
    const option: ChartOption = {
      ...
      series: series.map((entry) => ({ name: entry.imo, type: 'line', showSymbol: false, connectNulls: false, lineStyle: { color: vesselColor(entry.imo), width: 2 }, data: entry.points.map((point) => [point.timestamp, point.missing ? null : point.value]) })),
    }
    chart.setOption(option, { notMerge: true })
  }, [series, metric, validPointCount, dateRange.start, dateRange.end])
