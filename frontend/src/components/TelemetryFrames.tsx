...
export function TelemetryFrames({ vessels = [], focusedImo, selectedTimestamp, onFrameSelect, routeStart, routeEnd, onRouteStartChange, onRouteEndChange, goToTelemetryRequest }: Props) {
  ...
  return <section className="telemetry-frames" aria-label="Telemetry frames">
    <div className="frames-heading"><div><p className="eyebrow">04</p><h2>Daily navigation summary</h2></div><span>{vessels.length} vessel{vessels.length === 1 ? '' : 's'} · {summaries.length} daily summaries</span></div>
    <details className="route-filters"><summary>Filtres de trajectoire / Route filters</summary><div><label>Date de debut / Start <input type="date" value={routeStart} onChange={(event) => onRouteStartChange(event.target.value)} /></label><label>Date de fin / End <input type="date" value={routeEnd} onChange={(event) => onRouteEndChange(event.target.value)} /></label></div></details>
    <div className="frame-pagination"><button type="button" disabled={dates.length <= (page + 1) * 30} onClick={() => setPage((value) => value + 1)}>Older 30 days</button><span>{pageDates.at(-1)} to {pageDates[0]}</span><button type="button" disabled={!page} onClick={() => setPage((value) => Math.max(0, value - 1))}>Newer 30 days</button></div>
    <div className="frame-table-wrap"><table><thead><tr><th>Vessel</th><th>Date</th><th>Frames</th><th>First position</th><th>Last position</th><th>Distance</th><th>Avg SOG</th></tr></thead><tbody>
      {days.map((day) => { const key = `${day.imo}:${day.date}`; const first = day.records[0]; const last = day.records.at(-1)!; const focusedDay = day.imo === focusedImo && day.date === selectedDay; return <Fragment key={key}>
        ...
        {activeExpanded === key ? <tr className="frame-detail"><td colSpan={7}><table><thead><tr>{frameColumns.map((column) => <th key={column.key}>{column.label}</th>)}</tr></thead><tbody>{day.records.map((record) => <tr key={record.timestamp} className={record.timestamp === selectedTimestamp ? 'is-current' : ''} onClick={() => onFrameSelect(record.timestamp)}>{frameColumns.map((column) => <td key={column.key}>{format(record[column.key])}</td>)}</tr>)}</tbody></table></td></tr> : null}
      </Fragment> })}
    </tbody></table></div>
  </section>
}
