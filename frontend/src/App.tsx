...
import { replayFrames, nextReplayIndex } from './replay'
...
function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark"><Radio aria-hidden="true" size={18} /></span><div><p className="brand-name">{t('app.name')}</p><p className="brand-version">{t('app.version')}</p></div></div><div className="header-actions"><LanguageSwitcher /><button type="button" className="settings-button" aria-label="Open settings" aria-haspopup="dialog" onClick={() => setSettingsOpen(true)}><Settings size={17} /></button><button className="import-button" type="button" onClick={() => setImportOpen(true)}>{t('actions.import')}</button></div></header>
    {settingsOpen ? <div className="settings-modal-backdrop" role="presentation" onMouseDown={() => setSettingsOpen(false)}><section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title" onMouseDown={(event) => event.stopPropagation()}><div className="settings-modal-heading"><h2 id="settings-title">Workspace settings</h2><button type="button" aria-label="Close settings" onClick={() => setSettingsOpen(false)}>×</button></div><label>Replay speed<select value={playbackSpeed} onChange={(event) => setPlaybackSpeed(Number(event.target.value))}><option value={1}>1x</option><option value={2}>2x</option><option value={4}>4x</option></select></label><label>Map scale unit<select value={scaleUnit} onChange={(event) => setScaleUnit(event.target.value as 'km' | 'mi' | 'nm')}><option value="nm">Nautical miles</option><option value="km">Kilometers</option><option value="mi">Miles</option></select></label><ApiStatus /></section></div> : null}
    <main className="dashboard">
      ...
      <section className="workspace-grid" aria-label={t('dashboard.eyebrow')}>
       ...
       <section className="chart-metric-controls" aria-label="Telemetry chart metrics"><label>Primary metric<select value={activeMetric} onChange={(event) => setSelectedMetric(event.target.value)}>{metricsQuery.data?.map((metric) => <option key={metric.key} value={metric.key}>{metric.label}</option>)}</select></label></section>
        {chartSlotsWithSeries.map((slot, index) => <section className="chart-slot" key={`${slot.metric}-${index}`}>{index ? <div className="chart-slot-controls"><label>Chart metric<select value={slot.metric} onChange={(event) => setExtraChartMetrics((metrics) => metrics.map((metric, metricIndex) => metricIndex === index - 1 ? event.target.value : metric))}>{metricsQuery.data?.map((metric) => <option key={metric.key} value={metric.key}>{metric.label}</option>)}</select></label><button type="button" onClick={() => setExtraChartMetrics((metrics) => metrics.filter((_, metricIndex) => metricIndex !== index - 1))}>Remove chart</button></div> : null}<TelemetryChart title={index ? `Telemetry trend ${index + 1}` : 'Telemetry trend'} series={slot.series} loading={slot.loading} error={slot.error} colorVersion={colorRevision} dateRange={{ start: routeStart, end: routeEnd }} replayTimestamp={replay[replayValue]?.timestamp} onRetry={slot.refetch} /></section>)}
       <div className="chart-add-menu"><button type="button" className="chart-add-button" aria-expanded={chartMenuOpen} aria-controls="chart-metric-menu" onClick={() => setChartMenuOpen((open) => !open)}>+<span className="sr-only">Add metric chart</span></button>{chartMenuOpen ? <div id="chart-metric-menu" role="menu" aria-label="Add metric chart">{metricsQuery.data?.filter((metric) => !chartSlots.includes(metric.key)).map((metric) => <button type="button" role="menuitem" key={metric.key} onClick={() => { setExtraChartMetrics((metrics) => [...metrics, metric.key]); setChartMenuOpen(false) }}>{metric.label}</button>)}</div> : null}</div>
    </main>
  </div>
}
